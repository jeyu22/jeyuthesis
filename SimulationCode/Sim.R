...
library(broom)
library(broom.mixed)
library(SimDesign) # to quiet rcorrvar 
...
sim_mixedeffects <- function(dist, parameters, random_slopes = TRUE,
  ...
  if (random_slopes == TRUE) {
    r_effects <- rcorrvar(
      n = n.indiv, k_cont = 2, method = "Fleishman", means = c(m[1], m[1]), vars = c(m[2]^2, (m[2]^2)/18), skews = c(m[3], m[3]),
      skurts = c(m[4], m[4]),
  ...
  matrix_fixedeffects <- model.matrix(~time + treatment, data) # model matrix for fixed effects

  betas <- c(3.1, 0,0) # fixed effects coefficient vector
...
#t_p.value = 2*pt(abs(S_statistic), ddf = n.indiv * n.measurements - 3)))

# check if t and z pvalue is significant 
# group 
