...
hist(child_data$bmi, main = "Distribution of BMI Across Sample", xlab = "BMI")
```
...
```{r intercept, fig.cap= "Changes in BMI by Individual", fig.width = 4, fig.height=2.4}
...
```{r alcohol, fig.cap= "Changes in BMI by Individual and Alcohol Use", out.width = "49%"}
...
broom::tidy(lm.model) %>%
  kbl(caption = "Linear Model Output") %>%
...
intercept_model <- lmer(
  bmi ~ 1 + alcohol + age_at_start + (1 | AID) + wave,
  child_data
)
...
```{r compareintercept}
data.frame(cbind(term = S$term, Satterthwaite_p.value = S$p.value, KR_p.value = KR$p.value)) %>%
  kbl(caption = "Comparison of Fixed Effects P-Values in Random Intercept Model") %>%
  kable_classic(full_width = F) %>% kable_styling(latex_options = "HOLD_position")
```
...
```{r slopekr}
intercept_slope_model <- lmer(bmi ~ alcohol + real_age + wave + 
                                (1 + wave | AID), child_data)
...
ranova(intercept_slope_model)
drop1(intercept_slope_model)

anova(intercept_model, intercept_slope_model, refit = F)
```
...
```{r compareslope}
data.frame(cbind(term = S_s$term, Satterthwaite_p.value = S_s$p.value, 
