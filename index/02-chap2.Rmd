...
```{r}
all_sim_sum_treatment <-all_sim_sum %>%
    filter(KR_term == "treatment")

rbind(KR_err = mean(all_sim_sum_treatment$KR_t1err), S_err = mean(all_sim_sum_treatment$S_t1err),
Z_err = mean(all_sim_sum_treatment$Z_t1err), t_err =mean(all_sim_sum_treatment$t_t1err)) %>% kbl() %>% kable_classic()
```
Average Type I error rates are shown in (TABLE 1?) across all simulations. Despite a wide range of distributions, performance of methods align closely with previous studies examining only normal populations (@luke_evaluating_2017). T-as-z and standard degrees of freedom methods are much more anti-conservative compared to KR and Satterthwaite at these smaller sample sizes. Type I error rates are closest to .05 when using KR and Satterthwaite. After confirming that the simulation results follow similar patterns to what is expected, we proceed to examining more specific differences in performance of these methods among the nonnormal distributions. 
...
all_sim_sum %>%
    filter(KR_term == "treatment") %>%
  group_by(distribution, skew, kurtosis) %>%
  summarize(KR_type1 = mean(KR_t1err),S_type1 = mean(S_t1err), Z_type1 = mean(Z_t1err),
            t_type1 = mean(t_t1err)) %>%
  arrange(skew, kurtosis)%>% kbl() %>% kable_classic()
```
...
all_sim_sum %>%
    filter(KR_term == "treatment") %>%
  group_by(distribution,number_measurements) %>%
  summarize(KR_type1 = mean(KR_t1err),S_type1 = mean(S_t1err), Z_type1 = mean(Z_t1err),
            t_type1 = mean(t_t1err))%>% kbl() %>% kable_classic()
```
...
all_sim_sum %>%
  ...
  group_by(distribution,number_individuals,DF_method) %>%
  summarize(error_rate = mean(error_rate))%>%ggplot(aes(x = as.factor(number_individuals), y = error_rate, color = DF_method)) + facet_wrap(~distribution) +geom_point(alpha = .5) +geom_hline(yintercept = .05) + xlab("Sample Size") + ylab("Type I error rate") + labs(color='DF Method') 
```
...
all_sim_sum %>%
  ...
  add_header_above(c(" " = 3, "Sample Size" = 6)) %>%landscape()
```
...
all_sim_sum %>%
  ...
  add_header_above(c(" " = 4, "Random Intercept" = 1, "Random Slope" = 1,"Random Intercept" = 1,
                     "Random Slope" = 1,"Random Intercept" = 1,"Random Slope" = 1)) %>%
  add_header_above(c(" " = 4, "10" = 2, "18" = 2, "26" = 2)) %>%
  add_header_above(c(" " = 2, "Sample Size" = 8)) %>%landscape()
```
