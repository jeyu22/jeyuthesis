...
```{r fig4, fig.width= 6 ,fig.height = 6, fig.cap="Type I Error Rates by DF Method for Normal Distribution"}
 ggplot(aes(x = as.factor(number_individuals), y = error_rate, color = DF_method, 
            group = as.factor(number_measurements), ymin = error_rate-interval, ymax = error_rate+interval,), data = transformed_data %>% filter(distribution == "Gaussian")) +
   facet_grid(rslope ~ type) +
   geom_pointrange(alpha = .5, aes(shape = as.factor(number_measurements)), position = position_jitter(h = 0, w = 0.3)) +
   geom_hline(yintercept = .05) +
...
```{r fig5, fig.width= 6 ,fig.height = 6, fig.cap="Type I Error Rates Produced by KR vs Satterthwaite"}
 ggplot(aes(x = as.factor(number_individuals), y = error_rate, 
            color = DF_method, group = as.factor(number_measurements),ymin = error_rate-interval, ymax = error_rate+interval), data = transformed_data %>% filter(DF_method %in% c("KR_t1err_1", "S_t1err_1")))+
   facet_grid(rslope ~ type) +
   geom_pointrange(alpha = .4, aes(shape = as.factor(number_measurements)), position = position_jitter(h = 0, w = 0.1)) +
   geom_hline(yintercept = .05)+
   annotate("rect", xmin = -.1, xmax = 4, ymin = 0.025, ymax = .075, alpha = .4) +
   xlab("Number of Individuals") +
...
This stark contrast in performance between random slope and random intercept models across all distributions requires further investigation. @barr_random_2013 suggest that LMM with maximal effects are preferable and can reduce Type I error rates, as random slope models can account for between individual variation in slopes and reduce residual variance. However, in their study the distribution of the variable of interest in normal, and random effects were generated from a bivariate normal distribution. In this simulation, random effects for the intercept and time were drawn from two nonnormal distributions which can further complicate the distribution of the outcome variable and complicate the fit of the model. 
