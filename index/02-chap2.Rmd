...
```{r, echo = F}
par(mfrow=c(2,2))
curve(dlnorm(x, meanlog = 0, sdlog = .25), from = 0, to = 5,main = "Lognormal dist with meanlog 0 and sdlog .25")
curve(dlnorm(x, meanlog = 1, sdlog = .5), from = 0, to = 5,main = "Lognormal dist with meanlog 1 and sdlog .5")
x <- seq(0, 8, 0.1)
x_norm <- seq(-5,5,.1)
plot(x, dexp(x, .2),
  type = "l",
...
```{r, fig.width= 8 ,fig.height = 8, fig.cap="A nice image.",out.extra='angle=90'}
transformed_data <- all_sim_sum %>%
  filter(KR_term == "treatment") %>%
  mutate(KR_t1err_1 = paste(KR_t1err,KR_t1err_sd),
         S_t1err_1 = paste(S_t1err,S_t1err_sd),
         Z_t1err_1 = paste(Z_t1err,Z_t1err_sd),
         t_t1err_1 = paste(t_t1err,t_t1err_sd),
           ) %>%
  pivot_longer(cols = c("KR_t1err_1", "S_t1err_1", "Z_t1err_1", "t_t1err_1"), names_to = "DF_method", values_to = "error_rate_1") %>%
  mutate(type = paste(distribution, params, sep = ":"))
transformed_data[c('error_rate', 'sd')] <- str_split_fixed(transformed_data$error_rate_1, ' ', 2)
...
ggplot(aes(x = as.factor(number_individuals), y = error_rate, color = DF_method,
  ...
  geom_pointrange(alpha = .4, aes(shape = as.factor(number_measurements)), position = position_jitter(h = 0, w = 0.4)) + 
  geom_hline(yintercept = .05) +
  scale_y_continuous(limits = c(-.02, .3)) +
  annotate("rect", xmin = -.1, xmax = 4, ymin = 0.025, ymax = .075, alpha = .4) +
  scale_color_manual(labels = c("KR", "Satterthwaite","Standard DF","T-as-z"), values = c("red","green","purple","blue")) +
  xlab("Number of Individuals") +
  ylab("Error Rate") +
  labs(color = "DF Method", shape = "Number of Measurements") + theme(legend.position="bottom", legend.box = "vertical")
...
```{r, fig.width= 6 ,fig.height = 6, fig.cap="Exponential"}
...
## Lognormal 
```{r, fig.width= 6 ,fig.height = 6, fig.cap="Lognormal"}
...
## Normal Distribution 
```{r, fig.width= 6 ,fig.height = 6, fig.cap="Normal"}
 ggplot(aes(x = as.factor(number_individuals), y = error_rate, color = DF_method, 
...
```{r, fig.width= 6 ,fig.height = 6, fig.cap="KR vs Satterthwaite"}
 ggplot(aes(x = as.factor(number_individuals), y = error_rate, 
            color = DF_method, group = as.factor(number_measurements)), data = transformed_data %>% filter(DF_method %in% c("KR_t1err_1", "S_t1err_1"))) +
   facet_grid(rslope ~ type) +
   geom_point(alpha = .5, aes(shape = as.factor(number_measurements)), position = position_jitter(h = 0, w = 0.1)) +
   geom_hline(yintercept = .05)+
   annotate("rect", xmin = -.1, xmax = 4, ymin = 0.025, ymax = .075, alpha = .4) +
   xlab("Number of Individuals") +
   scale_color_manual(labels = c("KR", "Satterthwaite"), values = c("red","green")) +
   ylab("Error Rate") +
   labs(color = "DF Method", shape = "Number of Measurements")+ theme(legend.position="bottom", legend.box = "vertical")
```
