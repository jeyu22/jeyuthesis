...
{\centering \includegraphics[angle=90]{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-3-1} 

}

\caption{A nice image.}\label{fig:unnamed-chunk-3}
\end{figure}
...
{\centering \includegraphics{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-5-1} 

}

\caption{Lognormal}\label{fig:unnamed-chunk-5}
\end{figure}
...
Our first lognormal distribution with parameters \((0,.25)\) has lower values of kurtosis and skewness. DF methods appear to be positively impacted by increasing the sample size; this can be seen in both random intercept and random slope model, and the effect is compounded when there are 8 measurements as opposed to 4. In the random slopes model, one can see that the error rates produced by the \(t\)-as-\(z\) and standard DF method are reduced by one fourth when increasing the sample size from 10 to 26.
...
{\centering \includegraphics{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-6-1} 

}

\caption{Normal}\label{fig:unnamed-chunk-6}
\end{figure}
...
Tables for all error rates produced by 4 DF methods are located in APPENDIX?

\hypertarget{discussion}{%
\section{Discussion}\label{discussion}}

After comparing the error rates produced by DF methods across three distributions, sample sizes, and two numbers of measurements and complexity of models totaling to 96 unique conditions, we find that overall, KR and Satterthwaite DF methods yield the most robust error rates overall; in addition, increasing the number of measurements taken per individual also aids the DF methods in increasing robustness. Random slope models are significantly more complicated in terms of using DF methods to produce robust error rates, and also challenge the superiority of the KR method over the Satterthwaite method when looking at random slope models of different sample sizes. This stark contrast in performance between random slope and random intercept models across all distributions requires further investigation.@barr\_random\_2013 suggest that LMM with maximal effects are preferable and can reduce Type I error rates, as random slope models can account for between individual variation in slopes and reduce residual variance. One caveat is that the maximal model must be accurately justified by the design of the study. Since the predictors were arbitrarily constructed for the purpose of the simulation, perhaps the random effects structure imposed on the time variable was not appropriate based on the data.

In comparison to the effect that number of measurements had on the performance of DF methods, increasing the sample size was less likely to be associated with the DF method producing more robust Type I error rates; this trend is more apparent in random slope models, as well as distributions with larger skewness and kurtosis values. While increasing the number of individuals in a sample will help improve performance of DF methods overall, it is less imperative than increasing the number of measurements of each individual.

When comparing and constrasting performance of Satterthwaite and KR methods, which have both been demonstrated to be suitable options for producing robust Type I error, it seems that complexity of the model as well as sample size are two parameters that can differentiate their performance. In random intercept models, performance between the two methods are identical. In random slope models, KR is more conservative, and at a larger sample of size 26, it is too conservative in comparison to Satterthwaite. Given that KR is a further adjustment to Satterthwaite, its more conservative performance is not surprising, but its worse performance at larger samples is.

While previous studies comparing DF methods focus primarily on normal distributions, the results from this study demonstrate that applying DF methods to LMM with nonnormal distributions can still produce robust Type I error rates. However, careful consideration of the complexity of the model, number of measurements and individuals, as well as the skewness and kurtosis of the data must be considered, as they each have an effect on the DF method not only individual, but when interacting with other parameters as well. Overall, these results also indicate that Type I error rates are closest to .05 overall when employing Kenward-Roger and Satterthwaite methods.
\begin{itemize}
\tightlist
\item
  Should I keep table of type I error rates of every distribution?
\end{itemize}
...
Because we have repeated measurements of the same individual in this study, a regular linear model would not be able to capture the differences in variation between individuals and within individuals. We would hypothesize that changes in weight over time may vary less within a person over the years, as opposed to comparing changes in weight across two separate individuals. We first fit a linear model with no random effects for comparison. Then, we fit an intercept only model, allowing the intercept to vary by individual.
...
\begin{verbatim}
   ...
   Call:
   lm(formula = bmi ~ decisions_eat + run_away + cig + alcohol + 
       physical_fight + real_age + wave, data = child_data)
   
   Residuals:
      Min     1Q Median     3Q    Max 
   -5.402 -1.193 -0.100  0.766  6.235 
   
   Coefficients:
                   Estimate Std. Error t value Pr(>|t|)    
   (Intercept)       12.061      2.859    4.22  0.00023 ***
   decisions_eat1    -4.052      1.915   -2.12  0.04342 *  
   run_away3         -0.643      2.202   -0.29  0.77260    
   cig1              -0.819      1.867   -0.44  0.66450    
   alcohol1           5.115      1.128    4.53 0.000099 ***
   physical_fight1   -2.761      2.256   -1.22  0.23121    
   real_age           0.661      0.246    2.69  0.01192 *  
   wave              -0.995      1.021   -0.97  0.33818    
   ---
   Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
   
   Residual standard error: 2.7 on 28 degrees of freedom
   Multiple R-squared:  0.555,  Adjusted R-squared:  0.443 
   F-statistic: 4.98 on 7 and 28 DF,  p-value: 0.000934
\end{verbatim}
In a linear model with no random effects, we can see that having the freedom to make decisions about food, consumption of alcohol, and age are significant predictors of BMI. Being able to make decisions about food choices at the first wave of the study is associated with a 4 point decrease in BMI. Early exposure to alcohol is associated with a 5 point increase in BMI. Lastly, for every one year increase in age, BMI will increase by .66.
...
\begin{center}\includegraphics{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-14-1} \end{center}
\begin{center}\includegraphics{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-14-2} \end{center}
...
\begin{center}\includegraphics{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-17-1} \end{center}
\begin{center}\includegraphics{Jessica-Yu_StatThesis_files/figure-latex/unnamed-chunk-17-2} \end{center}
\begin{tabular}{l|l|l}
\hline
term & Satterthwaite\_p.value & KR\_p.value\\
\hline
