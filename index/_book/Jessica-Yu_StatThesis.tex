...
it can be shown that \(Cov(Y_{ij}, Y_{ik}) = g_{11} + (t_{ij} + t_{ik})g_{12} + t_{ij}t_{ik}g_{22}\) (Fitzmaurice et al., 2011).
...
The GLS estimator assumes that \(\Sigma_i\) is known. However, since this isn't usually the case, we can substitute \(\Sigma_i\) with a maximum likelihood estimate \(\hat\Sigma_i.\) It can be shown that the properties of \(\hat\beta\) still hold using an estimate of the covariance. These properties include \(\hat\beta\) being a consistent estimator of \(\beta\) and having sampling distribution that is multivariate normal with mean \(\beta\) and covariance \(\text{Cov}(\hat\beta) = \sum_{i=1}^{N}(X_i'\sum_i^{-1}X_i)^{-1}\) (Fitzmaurice et al., 2011).
...
\[E(b_i |Y_i) = GZ_i (\Sigma)^{-1}_i(Y_i-X_i\hat\beta).\] Because the covariance of \(Y_i\) is unknown, we can estimate both \(G\) and \(\Sigma_i\) using REML, creating \(\hat b_i\), also known as the empirical best linear unbiased prediction (BLUP) (Fitzmaurice et al., 2011). Thus, the equation for predicting the response profile is:
...
In the intercept only model, only one non-normal continuous variable is generated for the random effect, so the function \texttt{nonnormvar1()} is used. In order to generate measurement error, we simulate values from a \(e_{ij}\sim\mathit{N}(0,\sqrt{4}\sigma),\) where \(\sigma\) is the standard deviation from each distribution we want explore.
...
\hypertarget{code-from-all-chapters}{%
\section{Code From All Chapters}\label{code-from-all-chapters}}
\begin{Shaded}
\begin{Highlighting}[]
...
\KeywordTok{curve}\NormalTok{(}\KeywordTok{dlnorm}\NormalTok{(x, }\DataTypeTok{meanlog =} \DecValTok{0}\NormalTok{, }\DataTypeTok{sdlog =} \FloatTok{.25}\NormalTok{), }\DataTypeTok{from =} \DecValTok{0}\NormalTok{, }\DataTypeTok{to =} \DecValTok{5}\NormalTok{, }
      \DataTypeTok{main =} \StringTok{"Lognormal dist with meanlog 0 and sdlog .25"}\NormalTok{,}
      \DataTypeTok{cex.main =} \FloatTok{.6}\NormalTok{, }
...
\KeywordTok{curve}\NormalTok{(}\KeywordTok{dlnorm}\NormalTok{(x, }\DataTypeTok{meanlog =} \DecValTok{1}\NormalTok{, }\DataTypeTok{sdlog =} \FloatTok{.5}\NormalTok{), }\DataTypeTok{from =} \DecValTok{0}\NormalTok{, }\DataTypeTok{to =} \DecValTok{5}\NormalTok{, }
      \DataTypeTok{main =} \StringTok{"Lognormal dist with meanlog 1 and sdlog .5"}\NormalTok{,}
      \DataTypeTok{cex.main =} \FloatTok{.6}\NormalTok{,}
      \DataTypeTok{ylab =} \StringTok{""}\NormalTok{)}
...
\KeywordTok{plot}\NormalTok{(x, }\KeywordTok{dexp}\NormalTok{(x, }\FloatTok{.2}\NormalTok{),}
  ...
  \DataTypeTok{ylab =} \StringTok{""}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{, }\DataTypeTok{col =} \StringTok{"red"}\NormalTok{, }
  \DataTypeTok{main =} \StringTok{"Exponential dist with lambda = .2"}\NormalTok{,}
\DataTypeTok{cex.main =} \FloatTok{.6}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(x, }\KeywordTok{dexp}\NormalTok{(x, }\FloatTok{.9}\NormalTok{),}
  \DataTypeTok{type =} \StringTok{"l"}\NormalTok{,}
  \DataTypeTok{ylab =} \StringTok{""}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{, }\DataTypeTok{col =} \StringTok{"red"}\NormalTok{, }
  \DataTypeTok{main =} \StringTok{"Exponential dist with lambda = .9"}\NormalTok{,}
  \DataTypeTok{cex.main =} \FloatTok{.6}
\NormalTok{)}
...
\NormalTok{  ) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{pivot_longer}\NormalTok{(}\DataTypeTok{cols =} \KeywordTok{c}\NormalTok{(}\StringTok{"KR_t1err_1"}\NormalTok{, }\StringTok{"S_t1err_1"}\NormalTok{, }\StringTok{"Z_t1err_1"}\NormalTok{, }
                        \StringTok{"t_t1err_1"}\NormalTok{), }
...
\NormalTok{label_names <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}
                    \StringTok{`}\DataTypeTok{FALSE}\StringTok{`}\NormalTok{ =}\StringTok{ "Random Intercept"}\NormalTok{,}
                    \StringTok{`}\DataTypeTok{TRUE}\StringTok{`}\NormalTok{ =}\StringTok{ "Random Slope"}
\NormalTok{                    )}
...
\StringTok{  }\KeywordTok{kbl}\NormalTok{(}\DataTypeTok{caption =} \StringTok{"Random Effects for Random Intercept Model"}\NormalTok{,}
      \DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{digits =}\DecValTok{3}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_classic}\NormalTok{(}\DataTypeTok{full_width =}\NormalTok{ F)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"HOLD_position"}\NormalTok{)}
...
\StringTok{  }\KeywordTok{kbl}\NormalTok{(}\DataTypeTok{caption =} \StringTok{"Fixed Effects for Random Intercept Model Using Satterthwaite"}\NormalTok{, }
      \DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{) }\OperatorTok{%>%}
...
\NormalTok{broom.mixed}\OperatorTok{::}\KeywordTok{tidy}\NormalTok{(intercept_slope_model, }\DataTypeTok{ddf =} \StringTok{"Kenward-Roger"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(effect }\OperatorTok{==}\StringTok{ "ran_pars"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(group, term, estimate) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kbl}\NormalTok{(}\DataTypeTok{caption =} \StringTok{"Random Effects for Random Slope Model"}\NormalTok{,}
      \DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{digits =}\DecValTok{3}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_classic}\NormalTok{(}\DataTypeTok{full_width =}\NormalTok{ F)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"HOLD_position"}\NormalTok{)}
...
\NormalTok{S_s }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kbl}\NormalTok{(}\DataTypeTok{caption =} \StringTok{"Fixed Effects for Random Slope Model Using Satterthwaite"}\NormalTok{, }
      \DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_classic}\NormalTok{(}\DataTypeTok{full_width =}\NormalTok{ F) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"HOLD_position"}\NormalTok{)}
\KeywordTok{library}\NormalTok{(here)}
...
\chapter{Error Rates By DF Method}\label{pvalue-tables}}
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(here)}
\KeywordTok{library}\NormalTok{(tidyverse)}
...
\caption{\label{tab:unnamed-chunk-8}Error Rates for KR and Satterthwaite DF Methods}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-9}Error Rates for Standard DF and t-as-z Methods}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{r|r|l|r|r|r|r|r|r}
\hline
